...
using TreasureHunter.Server.ViewModels;

namespace TreasureHunter.Server.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser() { UserName = model.Email, Email = model.Email, DisplayName = model.DisplayName, Point = 100 };
        ...
        public ActionResult ChangeAvatar(HttpPostedFileBase file)
        {
            if (file != null)
            {
                ...
                string path = System.IO.Path.Combine(
                                       Server.MapPath("~/Upload/Avatar"), fileName);
                // file is uploaded
                file.SaveAs(path);
